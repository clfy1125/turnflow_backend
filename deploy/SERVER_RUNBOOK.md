...
# 2) llm.clfy.ai.kr 잠금: deploy/caddy/Caddyfile 의 llm 블록에서 IP 허용목록 or basic_auth 활성화
caddy reload --config /etc/caddy/Caddyfile     # 또는 서버의 Caddyfile 경로
...
서버 Caddyfile 을 `deploy/caddy/Caddyfile` 기준으로 교체(외부-IO/webhook 경로 prefix 를
실제 `config/api_urls.py`·`apps/pages/link_urls.py` 와 대조해 조정) → `caddy reload`.

**롤백(~15~30분):** `./deploy/scripts/rollback.sh` (이전 이미지로 워커/웹 복귀) +
Caddyfile 을 단일 업스트림으로 되돌리고 `caddy reload`. 마이그레이션은 가산적이라 되돌릴 필요 없음.
...
5. 서버 **Caddyfile 을 deploy/caddy/Caddyfile 기준으로 교체** + 경로 prefix 실제 URL 과 대조 → `caddy reload`.
