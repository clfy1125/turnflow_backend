...
2. **Cloudflare Origin Certificate** 발급(대시보드 → SSL/TLS → Origin Server → Create): **`*.clfy.ai.kr` + `clfy.ai.kr` 와일드카드 권장** — `turnflow-api.clfy.ai.kr`(신규 1단계) + `api.turnflow.clfy.ai.kr`(레거시) 둘 다 커버. → `origin.pem` + `origin.key` 저장. **SSL 모드 Full(strict) 확인.**
   - ⚠️ 콜로는 해외 IP 방화벽 때문에 Let's Encrypt **신규 발급 불가**(ACME secondary validation 타임아웃) → 콜로·GCP **둘 다 이 CF Origin cert 사용 + proxied 전제**. 엣지 cert는 Universal SSL `*.clfy.ai.kr` 가 1단계 `turnflow-api` 를 커버(2단계 `api.turnflow` 는 미커버였음).
