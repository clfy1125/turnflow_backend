...
- `docs/frontend/ADMIN_SNAPSHOT_ROSTER_RESPONSE.md` — 어드민 18차 회신(SNAP-1/2). 전체 현황 타일 → 회원 명단 `GET /admin/snapshot/paying|trial/`. **타일-명단 항등**을 위해 모수 쿼리를 `snapshot_rosters.py` 로 단일화 + 타일이 만든 **id→축 매핑을 스냅샷 캐시에 함께 얼려** 부분합(`?plan=`/`?bucket=`)까지 일치. **2026-09-23 부터 `no_card`(카드 없이 프로를 쓰는 체험자)도 SNAP-2 에 포함** — count == `trial_now.total`, `?bucket=no_card` 추가. 카드 없는 프로 30일 자동 지급이 체험 인원의 다수가 돼서, 빼두면 지금 프로를 쓰는 사람을 어드민에서 찾을 수 없었다. 금액 오독은 `expected_amount=null`(will_charge 에만 값) 로 막는다. ordering 화이트리스트 밖은 400, page_size 상한 500, `/admin/snapshot/**` 는 RBAC 미화이트리스트라 marketing_viewer 자동 403(PII 마스킹 미적용)
...
- `docs/frontend/EXTRA_IG_ACCOUNT_TRIAL_FRONTEND.md` — **체험 중 추가 IG 계정 0원 즉시 추가**(2026-08-21, 마이그 없음). 종전엔 `trialing` 이면 추가계정 구매/견적이 통째로 400 이라 프로 체험자가 계정을 못 늘렸다(prod CS #d34572b3 — 3분간 8회 시도). 체험 판정 단일 소스 = `compute_extra_accounts_charge`(TRIALING → 0원, 견적·실행 공유 → 견적=실청구 유지), 첫 결제 합산은 `tasks._renewal_amount_for` 가 원래 담당. 견적 응답에 `trial` 추가 — **`amount==0` 만으로는 "체험 무과금"과 "잔여 0일이라 0원"을 구별 못 한다**. ⚠️ 프론트 `QuoteGate` 가 400 사유를 버리고 "잠시 후 다시 시도"로 뭉개는 것이 이 사고의 절반이었다(재시도해도 영원히 실패) — 미납/해지예약/일시정지는 **여전히 400** 이라 그 화면은 아직 루프다. **체험 중 개수 상한은 의도적으로 두지 않았다**(2026-08-21 제품 결정 — 체험자가 10개를 0원으로 잡고 첫 결제 전 해지하면 월 99,000원어치가 무과금. 전환율 우선 + 실사용 0건이라 감수. **버그로 보고 상한을 넣지 말 것**) · 회신은 `docs/frontend/EXTRA_IG_ACCOUNT_TRIAL_RESPONSE.md`
- ⚠️ **billing 의 정책 거절 400 은 `{detail}` 단독으로 나갔었다** — `toss_views._flow_error_response` 가 DRF 예외 핸들러를 우회해 Response 를 직접 만들기 때문(§6 위반). 2026-08-21 부터 `detail` + `{success,error:{code,message}}` **동시 송출**로 고쳤다(`detail` 은 기존 프론트·앱이 읽으므로 영구 유지 — 지우면 결제 화면 문구가 한 번에 사라진다). 같은 엔드포인트의 시리얼라이저 `ValidationError` 는 원래부터 envelope 라 **한 URL 이 두 포맷을 내던 것**이 프론트가 밟은 함정이었다. 응답을 직접 만드는 다른 헬퍼도 같은 함정을 갖고 있는지 확인할 것
...
- `docs/frontend/CAMPAIGN_RESUME_REVIVE_FRONTEND.md` — **재개 시 정지 중 밀린 DM 자동 재발송**(2026-08-22, 마이그 없음). 종전엔 `resume` 이 status 만 ACTIVE 로 올려서 **정지 중 스킵된 건이 재개 뒤에도 영구 미발송**이었다(prod @pre2entt: 소급 발송 238건 직후 4분 만에 정지 → 191명이 재개 후에도 미수신, 운영자가 수동 revive). 이제 재개 경로 5곳(단건/일괄 resume · PATCH status=active · schedule activate · 어드민 재개)이 모두 `AutoDMCampaign.enqueue_paused_backlog_revive()` **단일 진입점**을 호출하고 응답에 `revive_queued` 를 담는다. 대상 판정 단일 소스 = `SentDMLog.revivable_paused_logs()`(정지 사유 스킵 + 창 안 = 댓글 7일/user_id 24h, 오래된 순, 1000건 상한). ⚠️ **스킵 사유 문구는 `SentDMLog.SKIP_REASON_CAMPAIGN_INACTIVE` 상수** — 리터럴로 복제/변경하면 되살림이 조용히 0건이 된다. 전이(비활성→active)일 때만 동작하므로 활성 캠페인 문구 수정 PATCH 에는 영향 없음. **전 플랜 허용**(프리미엄 `retry-failed` 와 달리 게이트 없음 — 무료는 월 200건 한도가 상한 역할)
...
- `docs/frontend/ACCOUNT_DELETION_AND_OAUTH_STATE_FRONTEND.md` — **탈퇴 차단 해제 + 소셜 로그인 state 유실**(2026-09-22, 마이그 없음). CS #247995c5 한 건에서 결함 3개가 나왔다. **백엔드 완료분**: `AccountDeleteView` 의 409 차단 조건에 **`has_billing_key` 를 추가** — 차단 근거인 *"잔여 유료기간 소멸 + 토스 빌링키 고아"* 는 **카드가 있을 때만 성립**하는데 `TRIALING` 이라는 이유로 **카드 없는 자동 프로체험**까지 막고 있었다. 그 결과 계정을 지우려던 사용자가 **원치 않은 구독 해지**를 먼저 했다(자동체험 217건 중 해지 5건인데 **4건이 가입 1시간 이내** — 이탈 지표 오염). 409 는 `detail` + envelope **동시 송출**(이 뷰도 DRF 핸들러를 우회한다) + `details.web_deletion_url`(구독 해지 없이 탈퇴되는 공개 경로). 테스트 `apps/authentication/tests/test_account_delete_gate.py`. **프론트 몫**: ①탈퇴 모달이 **401 외 전부를 generic 문구로 뭉갠다** — 백엔드 사유가 화면에 못 닿는다(`QuoteGate` 와 동형) ②`/delete-account` 는 배포돼 있는데 앱에 링크가 없어 **역대 사용 1건**·유예중 0건 ③**OAuth `state` 가 `sessionStorage`** 라 탭·웹뷰가 바뀌면 `expectedState=null` → `state_mismatch` **60건/36명**. ⚠️ **인스타는 백엔드가 이미 서버 검증**(`InstagramLoginState`)하므로 프론트 로컬 비교는 순수 중복 — 지우면 32건이 사라진다. ⚠️ **두 탈퇴 경로는 정책이 반대다** — `me/delete/`=즉시 하드삭제·구독중 409 / `deletion/*`=구독 자동해지·7일 유예. 앱 내 탈퇴에 "복구 가능" 이라고 쓰면 허위 고지가 된다
