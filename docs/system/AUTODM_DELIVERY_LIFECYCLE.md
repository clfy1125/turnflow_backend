...
### 3.2.2 공개 답글 서킷브레이커 (2026-08-24 재설계)

목적은 **인스타 제재(Action Block) 확산 차단 하나**다. 제재 중에 계속 답글을 시도하면 차단 기간이
연장되므로 스스로 멈춘다. 판정은 `tasks._public_reply_circuit_breaker()` 단일 함수.

| 항목 | 규칙 |
|---|---|
| 카운트 대상 | **제재성 코드만** — `1`(Action Block) · `190`(토큰만료) · `200`/`10`(권한·정책) |
| 카운트 제외 | **`code=100`** (댓글 삭제 `sub=33` · 7일 초과 · 잘못된 파라미터) — 사용자 행동/데이터 문제이지 제재가 아니다. **분자·분모 양쪽에서 뺀다** |
| 임계 | 10분 창에서 `제재성 실패 ≥ 3건` **AND** `제재성 실패 / (성공 + 제재성 실패) ≥ 0.5` |
| 범위 | **캠페인 단위**로 OFF. 단 `code=1`(계정 단위 제재)일 때만 그 IG 계정 전체 |
| 해제 | 수동 (사용자가 다시 켠다) |

> ⚠️ **옛 규칙이 만든 실제 사고 — prod CS #c13711f0 (@manjzangi, DM 로그 9,820건).**
> "10분 내 영구에러 **3건**(코드 무관) → **계정 전체** OFF" 였다. 그 계정의 영구에러 203건은
> **전부 `100/33`(댓글러가 자기 댓글 삭제)** 이고 Action Block·토큰만료는 **0건**인데, 서킷이
> **20번** 발동해 캠페인 공개답글을 최대 6개씩 껐다. 발송량이 많을수록 10분에 댓글 3개가
> 지워지는 건 일상이라 **헤비 유저는 사실상 반드시 걸리는** 구조였고, 사용자에겐 알림도 없어
> "중간중간 대댓글이 저절로 풀린다"로만 보였다.
> 테스트는 `apps/integrations/test_public_reply_circuit_breaker.py` — 옛 규칙이면 발동한다는
> 검출력 검증까지 포함한다.
