...
GET /billing/plans/  ──▶  requestBillingAuth(SDK)  ──▶  successUrl?authKey=...
             ...
             (쿠폰 시 30일+보너스, 과금 없음)     (200 + payment 객체)
```
...
- 쿠폰(제휴 코드) 입력 시 **30일 + 코드 보너스** 후 첫 결제. 코드는 **1인 1회**.
  보너스는 코드마다 다릅니다(14일·30일 등) → 총 일수는 `validate` 의 `total_trial_days` 로
  받아서 표기하세요. 숫자를 프론트에 하드코딩하지 마세요.
...
## 4. 쿠폰(제휴/레퍼럴 코드) — ⚠️ 2026-08-04 변경

**`POST /billing/referral/redeem/`(카드 없이 체험 시작)은 폐지됐습니다 — 항상 400.**
쿠폰은 **카드 등록 경로에만 동봉**하세요: `POST /billing/toss/confirm/` + `referral_code`.

이유: 무카드 경로가 기본 체험 30일을 가산하지 않아 **14일 쿠폰이 44일이 아니라 14일로**
나갔습니다. 자세한 내용과 프론트 수정 사항은 → [REFERRAL_COUPON_FRONTEND.md](REFERRAL_COUPON_FRONTEND.md)

- 총 무료 일수 = **30일 + 코드 보너스**. 표기는 `validate` 의 `total_trial_days` 사용
  (`trial_days` 는 보너스분만이라 그대로 노출하면 안 됩니다).
- 폐지 전 무카드 체험으로 시작한 기존 사용자가 카드를 등록(confirm)하면
  **잔여 체험 기간은 유지**되고 종료 시 첫 결제가 진행됩니다(`attach_only`).
- 제휴/레퍼럴 코드는 1인 1회입니다.
