...
> 연달아 터진 것이었습니다. 백엔드 몫은 **이미 고쳐 두었고**(§2), 이 문서는 프론트 몫(§3)입니다.
...
| # | 문제 | 담당 | 상태 |
|---|---|---|---|
| B-1 | 카드 없는 자동 프로체험이 탈퇴를 409로 막음 | 백엔드 | ✅ **수정 완료**(미배포) |
| B-2 | 409 응답에 `detail` 없음 → 프론트가 사유를 못 읽음 | 백엔드 | ✅ **수정 완료**(미배포) |
...
| **F-4** | 소셜 버튼이 "로그인"인지 "새 가입"인지 구분이 없음 | 프론트 | 제안 |
...
### F-4. "로그인"과 "새 가입"이 버튼에서 구분되지 않습니다 (제안)

이번 사용자는 **로그인하려다 신규 가입이 됐습니다.** 백엔드 매칭 규칙상 이건 막을 수 없습니다:

- `User.email` 은 **unique** 라 한 사람이 두 계정을 가지면 이메일이 반드시 다릅니다.
- 카카오 매칭은 `kakao_id` → **email 폴백**뿐입니다. 카카오 이메일이 기존 계정과 다르면
  **무조건 새 계정**이 생깁니다. (자동 연결을 넓히는 건 계정 탈취 표면이라 의도적으로 좁혀 둔 것입니다.)
- 그래서 **새 계정을 지워도 카카오 버튼을 다시 누르면 또 새 계정이 생깁니다.**

제안:

1. 가입이 **신규로 성사된 직후** 안내 한 줄 —
   *"처음 오셨네요! 이미 쓰던 계정이 있다면 이메일로 로그인해 주세요"*
   (`POST /auth/kakao/` 응답의 **`is_new_user`** 로 판별 — 이미 내려갑니다)
2. 로그인 모달에서 소셜 버튼 아래 *"예전에 이메일로 가입하셨나요?"* 링크를 눈에 띄게
