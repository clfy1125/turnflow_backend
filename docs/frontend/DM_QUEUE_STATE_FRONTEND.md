...
| 캠페인별 시간당 200건 설정 | 강제됨 | **제거됨** — 필드(`max_sends_per_hour`) 자체가 사라짐 |

**UX 함의**: "정시에 몰아서 나가고 그 사이 침묵"이 아니라, 댓글이 몰려도 **일정한 속도로 줄 서서
계속 나가는** 모델입니다. 그래서 "대기 N건 / 예상 완료 시각" 게이지가 정확하게 제공됩니다.

- `max_sends_per_hour` 필드: **완전히 제거됨**. 캠페인 생성/수정/조회 API 스키마에서 더 이상 존재하지 않고
  DB 컬럼도 드롭됐습니다. (구 프론트가 요청 바디에 실어 보내도 DRF 가 무시하므로 400 은 안 나지만, 페이로드에서 빼세요.)
  설정 UI 의 이 입력은 제거하세요.
