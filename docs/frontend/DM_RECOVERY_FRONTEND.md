...
3. 사용자가 요청함에서 메시지 요청 **수락** 후 **해당 게시물에 다시 댓글** → 복구 재발송.
   - **"재댓글"의 정의(2026-07-14 스코핑 확정)**: **캠페인이 명시한 그 게시물** 안이면
     내용·위치 무관 — 새 top-level 댓글이든, 안내 대댓글에 단 **스레드 답글**이든
     ("DM요청 수락했어요" 같은 임의 문구 포함) 전부 트리거된다.
   - **캠페인 키워드와 무관** — 복구 대기 중인 사용자 본인에게만 적용되므로 오남용 불가.
   - **다른 게시물의 댓글은 트리거하지 않는다** (같은 계정이라도). ANY_MEDIA(모든 게시물)
     캠페인은 캠페인 의미대로 모든 게시물에서 인정.
   - 복구 대기 건은 5분 수신자 쿨다운에서 면제되므로 안내를 보고 바로 재댓글해도 발송된다.
   - **웹훅 유실 보정**: 스레드 답글 재댓글의 웹훅이 유실돼도 시간당 폴링
     (`poll_recovery_recomments`)이 안내가 게시된 대기 건의 답글을 재조회해 자동 보정한다.
4. 재발송이 접수되면 이전 건이 `recovery_delivered` 로 승격. TTL(기본 7일) 내 성공이 없으면 `recovery_expired`.
   - 수락 **전에** 재댓글만 달면 재발송이 또 확정 실패하고 조용히 대기 상태로 남는다
     (안내 대댓글은 스팸 방지를 위해 반복 게시하지 않음) — 이후 수락하고 **또** 댓글을 달면 성공한다.
