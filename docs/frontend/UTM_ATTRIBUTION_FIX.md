...
# 추가 (2026-08-25 저녁) — 프론트 회신 반영, **우선순위 재조정**

[UTM_ATTRIBUTION_VERIFY_REQUEST.md](./UTM_ATTRIBUTION_VERIFY_REQUEST.md) 회신 8건 감사합니다.
답변 덕분에 **"가입 0" 의 원인 후보가 바뀌었습니다.** 위 §3 의 순서를 아래로 바꿔주세요.

## 회신으로 확정된 것

| 문항 | 답변 | 확정된 사실 |
|---|---|---|
| 1·2 | 비콘은 `MarketingLayout` 한 곳, `tf_vid` 생성도 거기뿐 | 앱 직행자는 `tf_vid` 가 **영구히 없다** → 가입 37% 조인 불가는 **계측 실패가 아니라 구조** |
| 3 | 같은 오리진, www 리다이렉트 없음 | localStorage 핸드오프는 정상 |
| 4 | 8/22~23 배포 없음 | 8/23 의 `visitor_id` 없는 가입 급증(16%→54%)은 **아직 원인 미상** |
| 5 | 인앱에서 **우리가** 구글 로그인을 막고 모달, Android 만 탈출 버튼 | 403 은 안 나지만 **구글 가입 경로가 인앱에서 차단됨** |
| 6 | CTA 가 `navigate('/home')` — **쿼리 드롭** | UTM 이 URL 에 안 남는다 → 브라우저를 바꾸면 **완전 소실** |
| 7 | 픽셀은 발사하나 `date_joined` 10분 휴리스틱 | → **백엔드가 해결했습니다** (아래 §6-3) |

## ⭐ 그래서 지금 제일 큰 문제 (백엔드 실측)

광고 집행 중 `POST /track/visit` 요청의 **User-Agent 를 세어봤습니다** (prod 액세스 로그, 3시간):
...
**광고 트래픽의 77%가 인앱 브라우저**입니다. 그리고 회신 5번대로 그 환경에서는 **구글 로그인이
막힙니다.** 그런데 최근 2일 신규 가입 20명 중 **14명이 구글 가입** — 우리 주력 가입 수단입니다.

정리하면: **주력 가입 수단이 광고 유입 경로에서 통째로 막혀 있습니다.**
인앱 iOS 7건 / Android 17건인데, **탈출 버튼은 Android 에만** 있습니다.

그리고 6번과 겹치면 이렇게 됩니다:

```
① 인스타 인앱: turnflow.link/?utm_source=meta…   ← UTM 있음, tf_vid 생성됨
② CTA 클릭 → navigate('/home')                  ← URL 에서 UTM 사라짐
③ 구글 버튼 → "인앱에서는 안 됩니다" 모달
④ (Android) 탈출 버튼 → 외부 브라우저가 여는 URL = 현재 URL = UTM 없음
⑤ 크롬: tf_vid 없음 · tf_attribution 없음 → 구글 가입 성공
⑥ 서버 기록: visitor_id 없음 · UTM 없음 · referrer=accounts.google.com → 채널 "direct"
```

**⑥은 우리가 8/24~25 에 실제로 관측한 10건의 패턴과 정확히 일치합니다.**

## 바뀐 우선순위

| 순위 | 항목 | 담당 | 효과 |
|---|---|---|---|
| **P0** | **§6-1 인앱 브라우저 가입 경로** | 프론트 | **전환율 직결 — 돈이 걸린 문제** |
| **P0** | **§6-2 CTA 가 UTM 을 URL 로 유지** | 프론트 | 브라우저가 바뀌어도 귀속 생존 |
| P1 | §3-1 캡처 덮어쓰기 가드 | 프론트 | 같은 브라우저 전환 귀속 (기존 요청) |
| P1 | §3-2 `eventID` — **§6-3 으로 방식 변경** | 프론트 | 픽셀 정확도 |
| P2 | §3-3 `fbclid`/`_fbp`/`_fbc` | 프론트 | CAPI 준비 |
| — | ~~§3-4 `tf_vid` 항상 생성~~ | — | **철회** — 회신대로 37% 가 줄지 않습니다 |

### §6-1 [P0] 인앱 브라우저에서 가입이 끝나게

지금은 구글 버튼을 막고 모달을 띄우는데, 그 모달이 **막다른 길**입니다(특히 iOS).
셋 중 하나 이상을 부탁드립니다:

1. **인앱 브라우저에서는 이메일 가입을 먼저·크게 노출** (회신대로 이메일은 정상 동작)
2. **iOS 에도 탈출 경로 제공** — iOS 는 자동 탈출이 불가하니 *"우측 상단 ⋯ → Safari로 열기"*
   이미지 안내라도 넣어주세요
3. **탈출 시 UTM 이 붙은 URL 로 내보내기** (§6-2 와 함께 하면 자동으로 해결됩니다)

### §6-2 [P0] CTA 가 UTM 을 URL 로 넘기게

`navigate('/home')` → `navigate({ pathname:'/home', search: window.location.search })`
정도로, **랜딩 내부 이동과 앱 진입 모두에서 쿼리스트링을 보존**해 주세요.

지금은 UTM 이 localStorage **단일 의존**이라, 브라우저가 바뀌는 순간(=광고 유입의 주 경로)
귀속이 100% 사라집니다. URL 에 남아 있으면 새 브라우저에서도 비콘이 돌아 **UTM·`tf_vid`
가 새로 잡힙니다.**

### §6-3 [P1] `is_new_user` — 휴리스틱을 버리세요 (백엔드 배포 완료)

회신 7번의 *"구글 가입은 `date_joined` 10분 휴리스틱이라 일부 누락 가능"* 은 서버가 풀어야
할 문제였습니다. `POST /api/v1/auth/google/` 응답에 **`is_new_user` (boolean)** 를 추가했습니다.
...
즉 `is_new_user === true` 인 요청과 가입 귀속 행이 1:1 로 대응합니다. 추정할 이유가 없습니다.

> `/auth/register/` 는 성공하면 **항상 신규**라 이 필드가 없습니다(200 이 아니라 201).

## 남은 숙제 — 4번(8/23 급증)

프론트 배포가 없었다면, 그 무렵 **랜딩을 거치지 않고 `/login` 으로 직행하는 유입**이 늘어난
것입니다. 외부 공유 링크·메일·카카오·검색결과 중 `/login` 을 직접 가리키는 것이 새로 생겼는지
짐작 가는 게 있으면 알려주세요. 이게 안 풀리면 앞으로도 가입의 절반이 계측 밖에 남습니다.

---

관련 문서: [SIGNUP_ATTRIBUTION_FRONTEND.md](./SIGNUP_ATTRIBUTION_FRONTEND.md) (방문→가입 귀속 계약 원본)
· [UTM_ATTRIBUTION_VERIFY_REQUEST.md](./UTM_ATTRIBUTION_VERIFY_REQUEST.md) (확인 요청서)
