...
| 구분 | 인원 | 첫 결제 |
|---|---|---|
| **30일 초과 체험(쿠폰) + 카드 등록** | **27명** | 가장 빠른 건 **2026-09-15** |
| 30일 체험 + 카드 | 7명 | 게이트 무관 |
| 카드 없는 체험 | 4명 | 대상 아님 |

이 27명은 **모달에서 동의하지 않으면 첫 결제가 되지 않고 무료로 전환**됩니다
(월 14,900원 × 27 = 402,300원). 대신 마진은 넉넉합니다:

- **오늘 막히는 사람 0명**, 오늘 나가는 메일 **0건** (14일 창에 든 사람이 아직 없음)
- `conversion_consent_required` 가 처음 `true` 로 바뀌는 시점 — **8월 16일경**(가장 빠른 사용자가 30일 창 진입)
- 첫 D-14 메일 — **9월 1일경**
- 첫 결제가 실제로 막히는 시점 — **9월 15일**

즉 **모달이 9월 초까지 나오면 아무 손실이 없습니다.** 8월 16일 이후에는 플래그가 `true` 로
내려가기 시작하니, 모달이 없더라도 화면이 깨지지는 않습니다(플래그를 무시하면 됨).
