...
- [LINK_IMAGE_BLOB_PERSIST_FRONTEND.md](frontend/LINK_IMAGE_BLOB_PERSIST_FRONTEND.md) — ⭐ **링크 페이지 이미지가 안 뜨는 원인 규명 + 프론트 요청서**(2026-09-22, 마이그 없음. CS #8f11703f 에서 발견). **F-1(핵심)** `BlockEditor.update()` 의 blob 가드가 `IMAGE_URL_FIELDS`(스칼라 4키) + `typeof==='string'` 이라 **`images[]`(배열)는 그냥 통과** → 800ms 디바운스 자동저장이 죽은 `blob:` URL 을 DB 에 박는다. ⚠️ blob 은 **업로드 중이 아니라 크롭 드래그 중**(`onGalleryLivePreview`)에 들어가므로 `uploading`·버튼 disabled 방어가 전혀 안 걸린다. 실피해 **prod 6건·공개 5건·5건 복구불가**(2026-04 건부터 있고 09-21 건이 최신 = 현재진행형). 만든 본인 화면엔 보이고 **방문자에게만 안 보인다**. **F-2** `images[]` ↔ `image_media_ids[]` 가 index-parallel 이 아니라(8↔2, 6↔[], 5↔[None,None,...]) **복구에 쓰면 엉뚱한 이미지가 박힌다** — `thumbnail_url` 은 형제 media_id 가 있어 번들 복구루틴이 살리지만 문자열 배열인 `images[]` 는 손도 못 댄다. 백엔드 결함 2건은 **현재 증상 없음**(실측: `/image-proxy` 200 `image/webp` ✅ · direct cors BLOCKED · `img crossOrigin` onerror · plain `<img>` 정상) — `.webp` 397건이 `application/octet-stream`(컨테이너에 `/etc/mime.types` 없음) · media 도메인 CORS 0·OPTIONS 403, 둘 다 §4. ⚠️ prod 에는 `/image-proxy` **하나뿐**(dev 우회 `/_r2` 는 `import.meta.env.DEV` 라 배포본에 0건)
