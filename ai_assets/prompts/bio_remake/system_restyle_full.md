...
- `page.data.design_settings` 전체 교체:
  ...
  - `blockBgColor` — 카드 톤
  - `fontFamily` — **기본 `Pretendard` 또는 `Noto Sans KR`**. 컨셉상 필요할 때만 `Nanum Myeongjo`(명조/세리프)·`IBM Plex Sans KR`·`Nanum Gothic`. **이 5개 외 폰트명 금지**(렌더 안 됨 → 기본 폰트 폴백). 상세는 블록 규칙의 "폰트 정책" 참고.
...
> ※ 위 'font 톤'은 **분위기 설명**일 뿐이다. 실제 `fontFamily` 는 **기본 `Pretendard`/`Noto Sans KR`**, 세리프 느낌이 필요하면 `Nanum Myeongjo` 를 쓴다. 모노스페이스 폰트는 없으니 그 느낌은 `custom_css`(letter-spacing 등)로 낸다.
