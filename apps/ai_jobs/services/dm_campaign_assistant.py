...
# Meta Instagram button template text 한도(640자). 버튼(링크/팔로우게이트)이 붙는 DM 문구는
# button template 로 나가 text 가 640자까지 허용된다. 안전망으로 opening/gate_prompt/reward 를
# 640자로 클립한다. (integrations.models.BUTTON_TEMPLATE_TEXT_MAX 와 동일 값 — 앱 간 결합을 피해 로컬 상수.)
_BUTTON_TEXT_MAX = 640
...
4. opening_dm — SIMPLE 모드 DM. 댓글 단 모두에게 즉시 보내는 첫 DM. 캠페인 목적/업종을 반영해 따뜻하고 브랜드톤 있게.
   2~4줄, 이모지 적당히. 게시물 혜택을 가볍게 언급해도 좋다. **640자 이내**(버튼 카드 한도).
...
5. follow_gate 묶음 — 버튼 게이트용. 아래 두 하위 모드 **모두에서 자연스럽게** 읽혀야 한다:
   ...
   - gate_prompt — 버튼이 달린 안내 DM. "댓글 감사 + 아래 버튼을 눌러 자료를 받아가세요" 톤. 2~3줄, 640자 이내.
   ...
   - reward_dm — 버튼 통과 후 보내는 본 DM. 약속한 실제 내용을 따뜻하게 2~4줄, 640자 이내.
...
def _output_schema_hint() -> str:
    return (
        ...
        '  "opening_dm": "SIMPLE 모드 DM 본문 (640자 이내, 링크 URL 넣지 말 것)",\n'
        '  "gate_prompt": "버튼 게이트 안내 DM (640자 이내)",\n'
        "  \"gate_button\": \"버튼 글자 (20자 이내, 중립: '자료 받기'/'받기')\",\n"
        '  "gate_button_alt": "팔로우 검증 모드용 대안 버튼 글자 (20자 이내, 예: \'팔로우했어요\')",\n'
        '  "reward_dm": "버튼 통과 후 본 DM (640자 이내, 링크 URL 넣지 말 것)",\n'
...
def _normalize_meta_fields(raw: dict, *, link_url: str, include_follow_gate: bool) -> dict:
    ...
    # opening DM — 링크 URL 은 본문에 넣지 않는다(버튼으로 첨부). 버튼 카드(button template)라 640자 이내.
    opening = _strip_link_lines(str(raw.get("opening_dm") or ""))
    if not opening:
        opening = "댓글 남겨주셔서 감사해요! 자세한 내용은 DM으로 보내드릴게요 😊"
    opening = _clip(opening, _BUTTON_TEXT_MAX, ellipsis=False)
    ...
    if include_follow_gate:
        ...
        prompt = _clip(prompt, _BUTTON_TEXT_MAX, ellipsis=False)
        ...
        reward = _clip(reward, _BUTTON_TEXT_MAX, ellipsis=False)
...
# ── 오프닝 DM 다양화 (톤·의미 유지 N개 변형) ──────────────────

DIVERSIFY_DEFAULT_COUNT = 10
DIVERSIFY_MIN_COUNT = 2
DIVERSIFY_MAX_COUNT = 30

_DIVERSIFY_SYSTEM = (
    "너는 한국어 인스타그램 마케팅 카피라이터다. 주어진 '오프닝 DM' 문구를 의미와 말투(톤)는 "
    "그대로 유지한 채 표현만 바꿔 서로 다른 변형을 만든다. 규칙: "
...
def diversify_opening(
    *,
    opening_message: str,
    count: int = DIVERSIFY_DEFAULT_COUNT,
    tone: str = "",
    model_name: str = "gemma-4",
    max_tokens: int = 4000,
    ...
    try:
        parsed = extract_json(primary.content)
        raw_list = parsed.get("variants") if isinstance(parsed, dict) else None
        ...
        logger.warning("diversify_opening: 파싱 실패 → 빈 변형. %s", exc)

    return DmDiversifyResult(
        variants=variants,
        requested_count=n,
        model=primary.model,
        elapsed_seconds=round(primary.elapsed_seconds, 3),
