...
4. opening_dm — SIMPLE 모드 DM. 댓글 단 모두에게 즉시 보내는 첫 DM. 캠페인 목적/업종을 반영해 따뜻하고 브랜드톤 있게.
   ...
   링크 맥락(LINK)이 주어졌으면 링크를 넣을 한 줄(예: "👉 {{link}}")을 포함한다. 링크 맥락이 없으면 링크 줄을 넣지 마라.
5. follow_gate 묶음 — 버튼 게이트용. 아래 두 하위 모드 **모두에서 자연스럽게** 읽혀야 한다:
   ...
   - reward_dm — 버튼 통과 후 보내는 본 DM. 약속한 실제 내용/링크를 담는다. 따뜻하게 2~4줄.
     링크 맥락이 있으면 링크 줄(예: "👉 {{link}}") 포함, 없으면 링크 줄 생략.
...
# 링크 표기
LINK(실제 URL)을 본문에 그대로 박지 말고, 링크 자리는 항상 `{{link}}` 플레이스홀더로 둔다(시스템이 치환).
링크 맥락이 없으면 `{{link}}` 자체를 쓰지 마라.
...
def _output_schema_hint() -> str:
    return (
        ...
        '  "opening_dm": "SIMPLE 모드 DM 본문 (2~4줄). 링크 맥락 있으면 {{link}} 줄 포함",\n'
        '  "gate_prompt": "버튼 게이트 안내 DM (2~3줄)",\n'
        "  \"gate_button\": \"버튼 글자 (20자 이내, 중립: '자료 받기'/'받기')\",\n"
        '  "gate_button_alt": "팔로우 검증 모드용 대안 버튼 글자 (20자 이내, 예: \'팔로우했어요\')",\n'
        '  "reward_dm": "버튼 통과 후 본 DM (2~4줄). 링크 맥락 있으면 {{link}} 줄 포함",\n'
...
def _sub_link(text: str, link_url: str) -> str:
    """{{link}} 치환. link_url 있으면 실제 URL 로, 없으면 플레이스홀더가 든 줄을 통째로 제거."""
    t = text or ""
    if (link_url or "").strip():
        return _LINK_RE.sub(link_url.strip(), t).strip()
...
def _normalize_meta_fields(raw: dict, *, link_url: str, include_follow_gate: bool) -> dict:
    """LLM 이 만든 항목(이름/키워드/DM 본문/게이트)을 정규화. dict 로 반환."""
    # name
    ...
    # opening DM
    opening = _sub_link(str(raw.get("opening_dm") or ""), link_url)
    if not opening:
    ...
    if include_follow_gate:
        prompt = _sub_link(str(raw.get("gate_prompt") or ""), link_url)
        if not prompt:
        ...
        reward = _sub_link(str(raw.get("reward_dm") or ""), link_url)
        if not reward:
