...
def _drop_fabricated_video_blocks(result: dict, stats: dict) -> None:
    """새-페이지 생성에서 video 블록을 제거한다.

    생성 모델은 실제 유튜브 URL 을 알 수 없어 **항상 가짜/플레이스홀더 URL**을 만든다
    (Rick Astley, watch?v=example, 삭제된 영상 ...). 그 결과 공개페이지에 "재생할 수 없음"
    에러박스나 빈 검은 사각형이 떠 페이지 신뢰도를 통째로 망친다. 깨진 임베드보다 **없는 게 낫다.**
    ...
    kept = []
    for b in blocks:
        if isinstance(b, dict) and (b.get("data") or {}).get("_type") == "video":
...
def sanitize_result_json(
    result: dict, *, long_text_ok: bool = False, drop_fabricated_video: bool = False
) -> dict:
    ...
    if drop_fabricated_video:
        ...
        _drop_fabricated_video_blocks(result, stats)
