...
⚠️ ``auto_action`` 은 지금 **API 로 내보내지 않는다**(어드민 화면에 구현여부 배지를 만들지
   ...
   구현 현황은 ``../../docs/system/DM_ERROR_POLICY_PLAN.md`` §3 체크리스트에서 관리한다.
...
_BY_CODE_SUBCODE: dict[tuple[str, str], dict] = {
    ...
    # ★ 2026-08-31 문구 전면 교정 — 종전 "계정을 지웠거나 차단한 경우"는 **틀렸다.**
    #   prod 실측으로 이 실패는 수신자의 속성이 아니라 **우리 발송 지연의 함수**임이 확인됐다.
    #   @use.ai.likejimin 두 캠페인(합계 1만 건)의 지연 구간별 실패율:
    #     0~10초  2,766건 → 2534014 **0건**(0.25% 는 전부 2534001)
    #     10~60초   945건 → 4.76%   /  3~10분 255건 → 9.02%  /  10분+ 153건 → 6.54%
    #     (다른 캠페인 5,922건도 같은 패턴 — 0~10초 0건, 10분+ 15.24%)
    #   같은 시간대(08~09시) 실패 8건 vs 성공 8건의 댓글을 Graph 로 직접 조회한 결과
    #   **실패 8/8 소멸 · 성공 8/8 생존**. 그리고 실패자 중 4명은 나중에 다시 댓글을 달아
    #   DM 을 정상 수신했다(= 계정이 살아 있다는 증거).
    #   → 즉 "발송 차례가 오기 전에 그 댓글이 지워진 것"이다. 지운 주체(본인/인스타 정리)는
    #     우리 데이터로 특정할 수 없지만, 레버는 어느 쪽이든 **발송 속도** 하나다.
    #   ⚠️ reason 키(recipient_not_found)는 프론트 저장 필터·dm_error_filters SQL·테스트가
    #      물고 있어 **문구만 바꾸고 키는 영구 고정**한다(사전 규약).
    ("100", "2534014"): _entry(
        R_RECIPIENT_NOT_FOUND,
        "발송 전에 댓글이 지워짐",
        "DM 을 보낼 차례가 되기 전에 그 댓글이 사라졌습니다. 비공개 답장은 댓글을 통해 "
        "보내므로, 댓글이 없어지면 Instagram 이 받는 사람을 찾지 못합니다"
        "(원문은 '요청한 사용자를 찾을 수 없음'으로 오지만 계정이 삭제된 것은 아닙니다). "
        "발송이 즉시 나간 건에서는 이 오류가 한 건도 나오지 않고, 대기가 몇 분씩 길어진 "
        "시간대에만 5~15% 로 올라갑니다 — 즉 댓글이 사라지기 전에 못 보낸 것입니다.",
        "개별 건은 되살릴 수 없습니다(댓글이 없어 비공개 답장 경로가 사라지고, 상대가 먼저 "
        "DM 을 보낸 적이 없어 24시간 창도 없습니다). 대신 캠페인의 **발송 지연**을 보세요 — "
        "댓글이 시간당 수백 건씩 몰리면 Instagram 의 시간당 한도(계정당 약 750건) 때문에 "
        "대기줄이 생깁니다. 이 숫자가 몰려 있으면 그 시간대에 발송이 밀린 것이므로, "
        "고객에게는 게시물 분산을 안내하세요.",
        NORMAL,
        # 수신자 사정이 아니라 '몰려서 제때 못 보냈다'가 사실이므로 안내 종류도 바꾼다.
        PEAK_NOTICE,
    ),
    ...
    # ⚠️ 이 번호는 **뜻이 두 개**다 (2026-08-31 prod 실측 110건 분해).
    #   (a) 진짜 게시물 차단 — 그 캠페인의 발송이 처음부터 100% 실패한다(실측 96시도 0성공,
    #       같은 계정 다른 캠페인은 524건 전부 성공). 댓글은 살아 있다.
    #   (b) 발송 전에 댓글이 지워진 것 — 나머지 14건. 같은 게시물에서 같은 시각에 수천 건이
    #       정상 발송되고, 실패 건의 댓글만 Graph 에서 사라져 있다(2534014 와 같은 원인).
    #   판정 규칙을 아직 코드로 옮기지 않았으므로(런타임 분기 미구현) 아래 조치문이 그
    #   구분법을 담는다. 나누게 되면 (b)는 policy=normal 로 내려가야 한다.
    ("200", "2534066"): _entry(
        R_POST_BLOCKED,
        "이 게시물만 자동 DM 이 막힘",
        "Instagram 이 이 게시물에 한해 자동 DM 을 막았을 때 나옵니다. 과거에 이 게시물로 "
        "(다른 툴로라도) 대량 발송한 이력이 있으면 주로 나타나고, 같은 계정의 다른 "
        "게시물은 정상 발송됩니다. 단 **발송 전에 댓글이 지워진 경우에도 같은 번호가 "
        "옵니다** — 그때는 게시물이 막힌 것이 아닙니다.",
        "먼저 둘을 가르세요. 그 캠페인의 성공 건이 하나도 없으면 게시물 차단이며 풀 수 "
        "없습니다 → 새 게시물로 옮기도록 안내하세요. 반대로 같은 시간대에 정상 발송이 "
        "많으면 '발송 전에 댓글이 지워짐'이라 조치할 것이 없습니다.",
        INVESTIGATE,
    ),
