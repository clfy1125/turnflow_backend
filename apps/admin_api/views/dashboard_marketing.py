        ...
        result.append(
            {
                ...
                "link": {"page": "/users", "params": {"id": owner_id}},
            }
        )
    return result
...
- **MRR 은 조회 시점 라이브 계산**: ACTIVE 유료 구독의
  ...
  (`extra_ig_accounts × 9,900원`). TRIALING 제외. 과거 재구성 불가 → `mrr.previous = null`.
...
- 응답은 Redis 에 **300초(5분) 캐시** (키 `admin:dash:mkt:{period}`).
