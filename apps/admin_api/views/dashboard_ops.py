...
- ``dm_quality`` / ``spam``: 윈도우 집계 + 제로필(zero-fill) 시계열
  (``dm_quality.failed`` 는 '확인 필요'만 — 숨겨진 요청·스팸/복구는 ``hidden_spam`` 으로 분리)
...
# failed(확인 필요): 사용자 조치가 필요한 '진짜' 실패.
...
#   (code=100/subcode=2534025). 실패가 아니라 '실패 DM 복구'의 대상 사유다.
#   복구 ON → recovery_pending/recovery_expired, 복구 OFF/미보유 → failed_param@2534025.
DM_HIDDEN_SPAM_Q = Q(
    status__in=(
...
def _dm_quality(until, since, granularity: str) -> tuple[dict, dict]:
    ...
    dm_agg = SentDMLog.objects.filter(created_at__gte=since).aggregate(
        ...
        recovery_delivered=Count("id", filter=Q(status=SentDMLog.Status.RECOVERY_DELIVERED)),
        ...
        # failed_param 중 비팔로워 채널 미개설(2534025)만 = 숨겨진 요청·스팸 (실패에서 분리)
        failed_param_hidden=Count(
            "id",
            filter=Q(status=SentDMLog.Status.FAILED_PARAM, error_subcode=HIDDEN_SPAM_SUBCODE),
        ),
...
def _risk_accounts(now) -> list[dict]:
    ...
    per_conn = (
        ...
        .annotate(
            ...
            read=Count("id", filter=Q(status=SentDMLog.Status.READ)),
            failed_no_trace=Count("id", filter=Q(status=SentDMLog.Status.FAILED_NO_TRACE)),
            # 2534025(숨겨진 요청·스팸)은 계정 건강 위험 신호가 아니므로 제외
            # (비팔로워 대상 콜드 오디언스에서 흔함 — repeated_param_errors 오탐 방지).
            failed_param=Count(
                "id",
                filter=Q(status=SentDMLog.Status.FAILED_PARAM)
                & ~Q(error_subcode=HIDDEN_SPAM_SUBCODE),
            ),
        )
    )
    for row in per_conn:
...
- `delivery_rate` 는 `/integrations/dm-verification/stats/` 와 동일 공식:
  `(delivered+read) / (accepted+delivered+read+failed_no_trace)`.
- **`dm_quality.failed` 는 '확인 필요'(진짜 실패)만 셉니다.** 비팔로워 채널 미개설로 첫 DM 이
  상대 숨김함/스팸함으로 간 건(`code=100/subcode=2534025`)과 그 복구 대기·만료
  (`recovery_pending`/`recovery_expired`)는 **실패가 아니라** `dm_quality.hidden_spam` 으로
