...
# ── 프롬프트에 **미리** 주는 금지어 표기 (JARGON_RULES 와 같은 순서) ──────────
# 왜: 게이트 반려 사유의 다수가 이 어휘 치환이었고, 4회 재작성 안에 못 끝나면 슬롯이 폴백돼
# 사용자는 템플릿 문장을 받게 됐다(실측: recommendations·strengths 폴백). 규칙을 뒤늦게
# 반려로 알려주는 대신 처음부터 알려주면 재작성이 줄어든다.
# ⚠️ 자유도는 줄지 않는다 — **어휘 치환 규칙**일 뿐 문장 구조·관점은 그대로 모델 몫이다.
# ⚠️ 각 표기가 **같은 인덱스의 규칙에 실제로 걸리는지** 테스트가 검증한다(프롬프트-게이트 드리프트 차단).
JARGON_PROMPT_WORDS = [
    "CTA",
    "캡션",
    "훅",
...
def jargon_prompt_block() -> str:
    """금지어 → 대체 표현 목록을 프롬프트용 텍스트로 만든다(게이트와 단일 소스)."""
    lines = [
        f"- {w} → {JARGON_RULES[i][1]}"
        for i, w in enumerate(JARGON_PROMPT_WORDS)
        if i < len(JARGON_RULES)
    ]
    return (
        "\n\n## 쓰면 반려되는 말 (미리 알려드립니다 — 대체 표현으로 쓰세요)\n"
        + "\n".join(lines)
        + "\n- 절반·두 배·수십만 같은 한글 수량어 → 숫자로 ('7개', '2배')\n"
        + "따옴표 안(영상 제목·댓글 원문 인용)은 예외입니다."
    )
...
JARGON_AUTOFIX = [
    ...
    # ⚠️ `\bCTA\b` 는 'CTA를' 을 못 잡는다 — 한글도 단어문자라 A→를 사이에 경계가 없다.
    #    영문 경계만 본다(게이트 쪽 패턴은 공백이 붙는 경우를 잡아 왔다).
    (re.compile(r"(?<![A-Za-z])CTA(?![A-Za-z])|씨티에이"), "댓글 유도"),
    (re.compile(r"훅|후킹|후크\b"), "첫 문장"),
    # 내부 enum 번역이 새어 나온 것 — 의도한 사람말 표현이 정해져 있다(feature_schema 라벨).
    (re.compile(r"말하는\s*얼굴"), "사람이 카메라를 보고 말하며 시작하는 영상"),
    (re.compile(r"인물\s*행동\w*|동작\s*시연형"), "손동작이나 시연 장면으로 시작하는 영상"),
    (re.compile(r"결과\s*제시\w*|결과\s*먼저형"), "결과물을 먼저 보여주는 영상"),
    (re.compile(r"손실\s*회피\w*"), "'모르면 손해'라고 말하며 시작하는 영상"),
    # '~형' 줄임말 — feature_schema.HOOK_LABEL_KO 에 사람말 이름이 이미 정해져 있다.
    # 실측(2026-08-04): `질문형` 하나 때문에 재작성 2·3회차를 통째로 낭비했다.
    (re.compile(r"질문형"), "질문으로 시작하는 영상"),
    (re.compile(r"설명형"), "바로 설명하며 시작하는 영상"),
    (re.compile(r"충격\s*경고형|경고형"), "놀랄 소식으로 시작하는 영상"),
    (re.compile(r"경험담형"), "경험담으로 시작하는 영상"),
    (re.compile(r"상황극형"), "공감되는 상황으로 시작하는 영상"),
    (re.compile(r"혜택\s*제공형"), "자료·혜택을 준다고 말하는 영상"),
    (re.compile(r"개수\s*예고형|리스트형|나열형"), "'몇 가지'라고 개수를 예고하는 영상"),
]
