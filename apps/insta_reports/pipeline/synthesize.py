...
_SCALE_GUIDANCE = {
    "starting": (
        ...
        "'분석'보다 **횟수와 실험**이 답입니다. 영상 수가 적어 어떤 비교도 우연일 수 있다는 점을 "
        "분명히 하고, 무엇을 몇 편 더 올려서 무엇을 확인할지의 **실험 계획**을 주세요. "
        "올리는 시간·자막 스타일을 미세하게 손보는 조언은 지금 단계에서 무의미합니다."
    ),
    "growing": (
        "이 계정은 **성장 중**입니다. 잘된 소수의 영상에서 **반복 가능한 공식**을 찾아내 "
        "그것을 굳히는 데 집중시키세요. 아직 규모가 작아 한두 편의 대박이 평균을 흔든다는 점을 "
        "감안해 '평소 조회수(가운데 값)' 기준으로 말하세요."
    ),
    "established": (
        "이 계정은 **자리 잡은 단계**입니다. 평균을 올리는 것보다 **하위 영상의 이유를 없애는 것**이 "
        "효율적입니다. 잘되는 패턴은 이미 있으니, 영상마다의 차이를 줄이는 쪽으로 조언하세요."
    ),
    "large": (
        "이 계정은 **대형**입니다(평소 조회수 10만 이상). 이미 도달은 충분하므로 "
        "'조회수를 늘리세요' 류의 조언은 **쓸모없습니다**. 대신 ①이 많은 조회수를 무엇으로 "
        "이어갈지(팔로워로 남기기·저장·외부 링크·판매) ②영상마다의 차이와 위험 관리 "
        "③반복 가능한 제작 순서 관점으로 조언하세요."
    ),
}
_REACH_GUIDANCE = {
    "explore_driven": (
        ...
        "조회수 늘리기가 아니라 '본 사람이 팔로워로 남게 하는 것'(프로필 첫인상·고정 게시물·"
        "시리즈화·영상 안에서 팔로우할 이유 제시)을 최우선으로 조언하세요.\n"
        "⚠️ 표현 주의: '팔로우 전환'·'전환율' 은 반려됩니다. '팔로워로 남게', "
        "'팔로우로 이어지게' 처럼 쓰세요."
    ),
    "follower_driven": (
