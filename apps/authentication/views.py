...
def _diagnose_login_failure(email, password):
    """authenticate()가 None을 반환한 이유를 추정한다 (개발 진단용)."""
    existing = User.objects.filter(email__iexact=email or "").first()
    if existing is None:
        return "해당 이메일의 사용자 없음 (미가입 / 이메일 오타)"
    if not existing.check_password(password or ""):
        return "비밀번호 불일치"
    if not existing.is_active:
        return "계정 비활성 (is_active=False)"
...
def _debug_log_login(request, email, response, reason, user=None):
    ...
    - 프로덕션(DEBUG=False)에서는 아무것도 남기지 않는다.
    - JWT 토큰/비밀번호 등 민감정보는 절대 로그에 남기지 않는다(토큰은 마스킹).
    """
    ...
    if isinstance(body, dict) and "tokens" in body:
        body = {**body, "tokens": "<redacted>"}

    logger.info(
        "[LOGIN-DEBUG] email=%s status=%s user_id=%s reason=%s body=%s",
        email,
...
class LoginView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if not email or not password:
            ...
            _debug_log_login(request, email, response, reason="email 또는 password 누락")
            return response
        ...
        if user is None:
            ...
            # DEBUG일 때만 실패 원인을 추정해 함께 남긴다 (운영에서는 호출되지 않음).
            reason = _diagnose_login_failure(email, password) if settings.DEBUG else None
            _debug_log_login(request, email, response, reason=reason)
            return response
        ...
        _debug_log_login(request, email, response, reason="로그인 성공", user=user)
        return response
...
| 409 | 유료 구독 중 — 먼저 구독 해지 필요 (`error.details.code == "active_subscription"`) |
    ...
    def delete(self, request):
        ...
        # 해지 완료(cancelled)여도 재개용 빌링키가 남아있을 수 있으므로 삭제 전 정리.
        if sub is not None:
            try:
                from apps.billing.tasks import _safe_delete_billing_key

                _safe_delete_billing_key(sub, reason="account_delete")
            except Exception:  # noqa: BLE001 - 빌링키 정리 실패가 탈퇴를 막으면 안 됨
                pass
