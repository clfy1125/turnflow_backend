...
class PageActivationView(APIView):
    ...
    apps/pages/views.py 참조). 선택한 페이지는 슬롯 확보 + 게시(is_public=True),
    미선택은 슬롯 반납(is_active=False)만 — is_public 은 보존한다. is_active=False 만으로
    노출은 차단되고, 업그레이드 복원 경로(_activate_all_pages 등)가 is_active 만 되살리므로
    is_public 을 함께 지우면 재업그레이드 후에도 페이지가 영구 비노출로 남는다.
    """
...
**슬롯 반납(is_active=False)만** — `is_public` 은 보존됩니다(트랜잭션으로 원자 적용).
`is_active=False` 만으로 외부 노출(`is_live`)은 차단되며, 이후 **업그레이드하면 자동으로
원래 게시 상태 그대로 복원**됩니다 (is_public 을 함께 지우면 복원이 불가능해짐).
...
| 401 | 토큰 없음/만료 |
    ...
    def post(self, request):
        ...
        # 적용: 선택 페이지는 활성 슬롯 + 게시, 나머지는 슬롯 반납(is_active=False)만.
        # 미선택의 is_public 은 보존 — is_active=False 만으로 노출(is_live)은 차단되고,
        # 업그레이드 복원(_activate_all_pages)이 is_active 만 되살리므로 is_public 을
        # 함께 지우면 재업그레이드 후에도 페이지가 영구 비노출로 남는다.
