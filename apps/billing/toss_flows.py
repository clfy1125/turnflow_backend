...
def change_extra_accounts(user, new_count: int) -> dict:
    ...
    - 증가: 증가분 × 9,900원을 현재 주기 잔여일만큼 비례 즉시 청구 → 성공 시 즉시 반영.
      ...
      **체험(TRIALING) 중이면 청구 0원**으로 즉시 반영하고, 체험 종료 후 첫 결제부터
      ...
      ⚠️ **체험 중에도 개수 상한을 따로 두지 않는다 — 의도된 제품 결정(2026-08-21).**
      `count` 상한은 시리얼라이저의 0~10 뿐이므로, 체험자가 10개를 0원으로 확보한 뒤
      첫 결제 전에 해지하면 월 99,000원어치 용량이 무과금으로 나간다. 이 구멍을 알고
      감수하기로 했다(전환율 우선 · 체험은 `trial_used_at` 로 1인 1회 · 카드 등록 필수라
      완전 익명 남용은 아니다). **버그로 보고 상한을 넣지 말 것** — 되돌리려면 제품 결정을
      먼저 뒤집어야 한다. 남용이 실제로 관측되면 그때 여기에 상한을 넣는다.
