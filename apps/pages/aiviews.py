...
## 재임포트 (같은 페이지 두 번)
이미 같은 외부 페이지를 임포트한 적이 있으면 기본적으로 **409 Conflict** 와
기존 페이지 정보를 돌려준다. 감지는 URL 문자열이 아니라 (소스, slug) 기준이라
인포크 신/구 도메인(``inpk.link`` ↔ ``link.inpock.co.kr``)처럼 표기가 달라도
같은 페이지면 잡힌다. 사용자가 새 페이지를 또 만들고 싶으면 ``force=true``.
...
| 서비스 | URL 패턴 | 추출 방식 |
|--------|----------|-----------|
| 인포크 | `https://link.inpock.co.kr/<slug>` 또는 `https://inpk.link/<slug>` | Next.js `__NEXT_DATA__` 파싱 |
| 리틀리 | `https://litt.ly/<alias>` | base64 인코딩된 임베드 JSON |
| 링크트리 | `https://linktr.ee/<username>` 또는 `https://linktree.com/<username>` | Next.js `__NEXT_DATA__` 파싱 |
...
```
                ...
                examples=[
                    OpenApiExample(
                        ...
                        value={
                            ...
                            "error": {
                                "code": 400,
                                "message": "지원 호스트가 아닙니다 (지원: link.inpock.co.kr, inpk.link, litt.ly, linktr.ee, linktree.com): https://example.com/foo",
                                "details": {"field": "url"},
                            },
                        },
                        response_only=True,
        ...
        examples=[
            ...
            OpenApiExample(
                "인포크 단축 도메인 임포트",
                summary="inpk.link 단축 주소도 동일하게 지원",
                value={"url": "https://inpk.link/hipservie"},
                request_only=True,
            ),
            OpenApiExample(
                "리틀리 임포트 + 제목 변경",
