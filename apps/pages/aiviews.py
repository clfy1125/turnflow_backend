...
def _capture_original_if_missing(page: Page, user) -> PageSnapshot | None:
    ...
    원본으로 되돌릴 수 있어야 하므로.
    """
    if PageSnapshot.objects.filter(
        page=page, reason=PageSnapshot.Reason.AI_EDIT
    ).exists():
        return None
...
def _upsert_latest_ai_result(page: Page, user) -> PageSnapshot:
    """AI 편집 직후의 결과 상태를 페이지당 1개 슬롯에 덮어쓰기 (upsert).
...
def _restore_from_snapshot(page: Page, snap: PageSnapshot) -> None:
    ...
    id_map = {old: new.id for old, new in zip(old_ids, created) if old is not None}
    if id_map:
        to_update = []
...
| 404 | 페이지 없음 또는 다른 사용자의 페이지 |
    ...
    def post(self, request, slug: str):
        ...
        # 변경 후: 최신 AI 작업물 슬롯에 결과 상태 upsert (토글용)
        _upsert_latest_ai_result(page, user=request.user)
...
class AiPageSnapshotListView(APIView):
    ...
    @extend_schema(
        tags=[_AI_TAG],
        summary="페이지 스냅샷 조회 (원본 / 최신 AI 작업물)",
        description="""
## 개요
`POST /api/v1/pages/ai/@{slug}/` (AI 1-shot 편집) 가 호출되면 두 슬롯이
자동 관리됩니다:

| reason | 의미 | 갱신 시점 |
|--------|------|-----------|
| `ai_edit` | AI 첫 호출 직전의 **원본** | 첫 호출 시 1회 |
| `latest_ai_result` | 가장 최근 AI 편집 직후의 **작업물** | AI 호출마다 upsert |

이 두 스냅샷 사이를 ``restore`` 로 자유롭게 토글할 수 있고, ``restore`` 는
어느 쪽도 지우지 않습니다.
...
## 보관 정책
- 페이지당 reason 별 **최대 1건** (DB 레벨 unique 보장).
- 사용자가 원본으로 돌아간 뒤에도 ``latest_ai_result`` 는 살아 있어 다시 작업물로 돌아갈 수 있음.
...
class AiPageSnapshotRestoreView(APIView):
    ...
    @extend_schema(
        ...
        description="""
## 개요
지정한 스냅샷 (``ai_edit`` 원본 또는 ``latest_ai_result`` 최신 작업물) 으로
페이지를 **원자적으로 복원**합니다. 복원 후에도 두 슬롯 모두 그대로
유지되므로 원본 ↔ 작업물 토글이 가능합니다.
...
## 동작 순서
1. 페이지 메타 (`title`, `is_public`, `data`, `custom_css`) 를 스냅샷 값으로 덮어쓰기
2. 현재 블록 전체 삭제 후 스냅샷의 블록을 ``order`` 그대로 재생성
3. 폴더/토글 블록의 ``child_block_ids`` 가 옛 block.id 를 가리킬 경우, 새 ID 로 재매핑
4. 전체가 단일 트랜잭션 — 중간 실패 시 모두 롤백
5. 스냅샷 슬롯은 그대로 유지 — 반대 슬롯으로 재복원 가능
