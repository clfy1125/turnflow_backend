...
# 형식: "{숨김함 고지} {중간 이모지} {수락+재댓글 유도}"  (+ 50% 확률로 뒤에 {끝 이모지})
# 캠페인이 recovery_reply_templates(리스트)를 지정하면 그게 우선, 없으면 이 조합기를 쓴다.
#
# v2 메커니즘(2026-07-14): 2534025 실패 시 DM 은 대부분 수신자의 '숨겨진 요청/스팸함'에
# 들어가 있거나, 요청 수락 전이라 채널이 안 열린 상태다. "DM 주세요"(인바운드 트리거)는
# 먼저 온 DM 수신을 꺼둔 사용자에게 동작하지 않아 폐기 → "요청함 확인·수락 후 **다시 댓글**"
# 로 유도한다. 재댓글은 기존 댓글→DM 경로를 그대로 타므로 별도 수신 감지가 필요 없다.
RECOVERY_FIRST_PHRASES = [
    "DM이 숨겨진 요청함으로 들어갔어요",
    "보낸 DM이 메시지 요청함에 있어요",
    "DM이 숨겨진 요청이나 스팸함으로 갔어요",
    "메시지가 요청함으로 분류된 것 같아요",
    "DM이 스팸함으로 들어간 것 같아요",
    "보내드린 DM이 숨겨진 요청함에 있을 거예요",
    "DM이 메시지 요청함으로 갔네요",
    "디엠이 숨겨진 요청함으로 들어갔네요",
    "DM이 요청함에 숨어있는 것 같아요",
    "메시지가 숨겨진 요청으로 분류됐어요",
    "보낸 메시지가 요청함에 대기 중이에요",
    "DM이 스팸함이나 요청함으로 간 것 같아요",
    "디엠이 메시지 요청함에 도착해 있어요",
    "DM이 숨겨진 요청함에 잠들어 있어요",
    "메시지가 스팸함으로 분류된 것 같네요",
    "보내드린 DM이 요청함에 있는 것 같아요",
    "DM이 숨겨진 메시지 요청으로 들어갔어요",
    "디엠이 요청함으로 분류돼 버렸어요",
    "DM이 바로 안 보이고 요청함에 있을 거예요",
    "메시지가 숨겨진 요청함 쪽으로 갔어요",
    "DM이 일반 채팅이 아닌 요청함으로 갔어요",
    "보낸 DM이 스팸함에 들어간 것 같아요",
    "디엠이 숨겨진 요청 목록에 있어요",
    "DM이 메시지 요청 쪽으로 분류됐어요",
    "메시지가 요청함에 숨어버렸네요",
    "DM이 숨겨진 요청함으로 배달됐어요",
    "보내드린 디엠이 요청함에 대기 중일 거예요",
    "DM이 인스타 요청함으로 들어가 버렸어요",
    "메시지가 숨겨진 요청/스팸함으로 간 것 같아요",
    "DM이 채팅 목록 대신 요청함에 있어요",
    "디엠이 스팸함으로 새어 들어간 것 같아요",
    "DM이 메시지 요청함에서 기다리고 있어요",
    "보낸 메시지가 숨겨진 요청함에 있네요",
    "DM이 요청함으로 숨어들어 갔어요",
    "메시지가 바로 안 보이고 요청함에 있을 거예요",
    "DM이 숨겨진 요청함이나 스팸함에 있을 거예요",
    "디엠이 메시지 요청으로 분류된 것 같아요",
    "DM이 요청함 쪽에 도착해 있는 것 같아요",
]
RECOVERY_MID_EMOJIS = ["🥲", "😢", "😥", "😭", "😔", "🙈", "😅", "🥺"]
RECOVERY_CLOSER_PHRASES = [
    "메시지 요청 수락하시고 다시 댓글 남겨주시면 바로 보내드릴게요!",
    "요청함에서 수락 후 댓글 한 번만 더 달아주시면 다시 보내드려요!",
    "숨겨진 요청함 확인·수락하시고 재댓글 주시면 즉시 다시 보내드릴게요!",
    "요청 수락하신 뒤 다시 댓글 달아주시면 바로 재발송해드려요!",
    "메시지 요청함에서 수락하고 댓글 다시 남겨주세요, 바로 보내드릴게요!",
    "요청함 확인하시고 댓글 한 번 더 달아주시면 다시 챙겨드릴게요!",
    "수락하신 다음 재댓글 남겨주시면 곧바로 다시 보내드릴게요!",
    "숨겨진 요청 수락 후 댓글 다시 달아주시면 재전송해드려요!",
    "요청함에서 수락하고 다시 댓글 주시면 바로 다시 보내드립니다!",
    "메시지 요청 수락 후 댓글 한 번만 더 부탁드려요, 바로 보내드릴게요!",
    "요청함 확인 후 재댓글 남겨주시면 즉시 다시 보내드려요!",
    "수락하시고 댓글 다시 달아주시면 자료 바로 다시 드릴게요!",
    "숨겨진 요청함에서 수락 후 댓글 남겨주시면 다시 보내드릴게요!",
    "요청 수락하고 한 번 더 댓글 주시면 곧바로 재발송할게요!",
    "메시지 요청함 확인하시고 재댓글 주시면 다시 보내드려요!",
    "수락 후 다시 댓글 달아주시면 바로 다시 챙겨드릴게요!",
    "요청함에서 수락하신 뒤 댓글 한 번 더 남겨주세요, 재전송해드려요!",
    "숨겨진 요청 확인·수락하고 다시 댓글 주시면 바로 보내드릴게요!",
    "요청 수락하신 다음 댓글 다시 달아주시면 즉시 보내드립니다!",
    "메시지 요청 수락하고 재댓글 남겨주시면 다시 보내드릴게요!",
    "요청함 수락 후 댓글 한 번만 더 달아주시면 바로 드릴게요!",
    "수락하시고 다시 댓글 남겨주시면 자료 재발송해드릴게요!",
    "숨겨진 요청함 수락 후 재댓글 주시면 곧바로 다시 보내드려요!",
    "요청 확인·수락하시고 댓글 다시 달아주시면 바로 재전송할게요!",
    "메시지 요청함에서 수락하신 뒤 재댓글 부탁드려요, 다시 보내드릴게요!",
    "수락 완료 후 댓글 한 번 더 남겨주시면 즉시 재발송해드려요!",
    "요청함에서 수락하고 댓글 다시 주시면 다시 챙겨서 보내드릴게요!",
    "숨겨진 요청 수락하신 다음 다시 댓글 달아주시면 보내드립니다!",
    "요청 수락 후 재댓글 남겨주시면 바로 다시 보내드릴게요!",
    "메시지 요청 확인·수락하고 댓글 한 번 더 부탁드려요, 재전송해드릴게요!",
    "수락하신 뒤 다시 댓글 주시면 자료 곧바로 다시 드릴게요!",
    "요청함 확인 후 댓글 다시 남겨주시면 바로 재발송해드려요!",
    "숨겨진 요청함에서 수락하고 재댓글 주시면 다시 보내드릴게요!",
    "요청 수락하시고 댓글 한 번만 더 달아주세요, 즉시 보내드릴게요!",
    "메시지 요청 수락 후 다시 댓글 남겨주시면 재전송해드립니다!",
    "수락하고 재댓글 달아주시면 바로 다시 챙겨드릴게요!",
    "요청함에서 수락하신 후 댓글 다시 주시면 곧장 보내드려요!",
    "숨겨진 요청 확인하시고 댓글 한 번 더 남겨주시면 다시 드릴게요!",
    "요청 수락 완료 후 다시 댓글 달아주시면 바로 재전송할게요!",
]
...
def compose_recovery_reply() -> str:
    ...
    "{숨김함 고지} {중간 이모지} {수락+재댓글 유도}" 조합 + 50% 확률로 뒤에 " {끝 이모지}".
    """
    import random
...
class AutoDMCampaign(models.Model):
    ...
    # ===== 실패 DM 복구 (recovery) =====
    # 비팔로워가 댓글만 달면 opening 비공개답글이 채널 미개설로 실패(code=100 subcode=2534025)하고,
    # DM 은 대부분 수신자의 '숨겨진 요청/스팸함'에 들어가 있다.
    # v2(2026-07-14): 확정 실패 시 대댓글로 "요청함 확인·수락 후 다시 댓글" 을 안내하고,
    # 같은 사용자가 **다시 댓글을 달면** 일반 댓글→DM 경로가 재발송한다(성공 시 이전
    # RECOVERY_PENDING 을 RECOVERY_DELIVERED 로 자동 승격). 인바운드 DM 감지 방식(v1)은
    # 'DM 먼저 받기'를 꺼둔 사용자에게 동작하지 않아 폐기했다.
    recovery_reply_enabled = models.BooleanField(
        default=True,
        verbose_name="실패 DM 복구 사용",
        help_text=(
            "opening 비공개답글이 2534025(비팔로워 채널 미개설)로 **확정** 실패하면 완전 실패로 "
            "두지 않고, 댓글에 'DM이 숨겨진 요청/스팸함으로 갔어요 — 수락 후 다시 댓글 달아주세요' "
            "안내 대댓글을 남긴다. 사용자가 요청을 수락하고 다시 댓글을 달면 일반 발송 경로로 "
            "재발송되며, 성공 시 이전 실패 건은 자동으로 성공 처리된다. **프로 전용**(dm_recovery "
            "기능) — 미보유 플랜에서는 켜져 있어도 동작하지 않고 해당 DM 은 일반 실패로 종결된다."
        ),
    )
    recovery_reply_templates = models.JSONField(
        ...
        help_text=(
            "실패 시 무작위로 1개 골라 댓글에 게시. 봇 검사 회피를 위해 여러 문구 권장. 비우면 "
            "서버 조합기가 매번 새 문구 생성. "
            "예: ['DM이 숨겨진 요청함으로 갔어요 🥲 수락하시고 다시 댓글 남겨주시면 바로 보내드릴게요!']"
        ),
    )
    recovery_keyword = models.CharField(
        ...
        help_text=(
            "(deprecated v2 — 인바운드 DM 트리거 폐기로 더 이상 사용되지 않음. "
            "API 하위호환을 위해 필드만 유지하며 값은 무시된다.)"
        ),
    )
    recovery_ttl_seconds = models.IntegerField(
        ...
        help_text=(
            "RECOVERY_PENDING 진입 후 이 시간까지 같은 사용자의 재댓글 발송 성공이 없으면 "
            "RECOVERY_EXPIRED 로 만료(완전 실패 정산). 기본 7일(댓글 비공개답글 창과 동일). "
            "배치/쿨다운은 공개답글 설정을 공유한다."
        ),
    )
    ...
    def matches_recovery_keyword(self, message_text: str) -> bool:
        """(deprecated v2) 인바운드 DM 텍스트가 복구 트리거 조건에 맞는지.

        인바운드 DM 트리거 폐기(재댓글 방식 전환)로 발송 경로에서는 더 이상 호출되지 않는다.
        recovery_keyword 필드와 함께 API 하위호환용으로만 유지.
        """
...
class SentDMLog(models.Model):
    ...
    class Status(models.TextChoices):
        ...
        RECOVERY_PENDING = "recovery_pending", "복구 대기 (안내 답글 게시, 수락·재댓글 대기)"
        RECOVERY_DELIVERED = "recovery_delivered", "복구 재전송 성공"
        RECOVERY_EXPIRED = "recovery_expired", "복구 대기 만료 (수락·재댓글 없음)"
